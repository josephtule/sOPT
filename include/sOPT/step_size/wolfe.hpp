...
inline StepAttempt wolfe_impl(
    ...
    if (!finite_neg(g0p)) return StepAttempt::line_search_failed; // must be descent
    ...
    auto wolfe_ok = [&](f64 a, f64 ft, f64 dft) -> bool {
        if (!(ft <= f0 + c1 * a * g0p)) return false; // Armijo/Sufficient Decrease
        if constexpr (Strong) {
    ...
    auto zoom = [&](f64 alo, f64 ahi, f64 flo, f64 fhi, f64 d_lo) -> StepAttempt {
        if (!(alo < ahi)) return StepAttempt::line_search_failed;
        if (!isfinite(d_lo)) return StepAttempt::line_search_failed;
        ...
        for (i32 k = 0; k < opt.ls.max_iters; k++) {
            ...
            if ((f_next > f0 + c1 * aj * g0p) || (f_next >= flo)) { // Armijo
                ahi = aj;
                fhi = f_next;
                continue;
            }
