...
inline StepAttempt wolfe_interp_impl(
    OracleT& oracle,
    ecref<vecXd> x,
    f64 f0,
    ecref<vecXd> g0,
    ...
    if (!in_op(c2, c1, 1.0)) return StepAttempt::line_search_failed;
    const f64 alpha_max = opt.ls.alpha_max;
    if (!finite_pos(alpha_max)) return StepAttempt::line_search_failed;
    ...
    auto zoom = [&](f64 alo, f64 ahi, f64 flo, f64 fhi, f64 dlo) -> StepAttempt {
        if (!(alo < ahi)) return StepAttempt::line_search_failed;
        if (!isfinite(dlo)) return StepAttempt::line_search_failed;

        for (i32 k = 0; k < opt.ls.max_iters; k++) {
            const f64 aj = detail::interpolated_zoom_trial(alo, flo, dlo, ahi, fhi);
    ...
    for (i32 i = 0; i < opt.ls.max_iters; i++) {
        ...
        if ((f_next > f0 + c1 * alpha * g0p) || (i > 0 && f_next >= f_prev)) {
            return zoom(a_prev, alpha, f_prev, f_next, d_prev);
        }
        ...
        if (wolfe_ok(alpha, f_next, dphi_a)) return StepAttempt::accepted;

        if (dphi_a >= 0.0) return zoom(a_prev, alpha, f_prev, f_next, d_prev);
...
struct WolfeWeakInterp {
    ...
    StepAttempt operator()(
        ...
        return wolfe_interp_impl<false>(oracle, x, f0, g0, p, alpha, x_next, f_next, opt);
    }
};

struct WolfeStrongInterp {
    template <typename OracleT>
    StepAttempt operator()(
        OracleT& oracle,
        ecref<vecXd> x,
        ...
        return wolfe_interp_impl<true>(oracle, x, f0, g0, p, alpha, x_next, f_next, opt);
    }
};
